...
public class CoordinateScrolls {
	...
	/* the observatory center Location */
	...
	public static enum CoordinateData {
		...
		private Location diggingPosition;
		private static Map<Integer, CoordinateData> clues = new HashMap<Integer, CoordinateData>();

		private static Map<Location, CoordinateData> positions = new HashMap<Location, CoordinateData>();

		static {
			for (final CoordinateData data : CoordinateData.values()) {
		...
		public static CoordinateData forIdPosition(Location position) {
	...
	public static final Location OBSERVATORY_Position = new Location(2440,
			3161, 0);
	...
	public static String[] calculateActualPosition(int x, int y) {
		final int obsX = OBSERVATORY_Position.getX();
		final int obsY = OBSERVATORY_Position.getY();
		final int differenceX = x - obsX;
		final int differenceY = y - obsY;
		final double minutesX = Math.abs(differenceX) * ONE_MINUTE_TO_SQUARE;
		final double minutesY = Math.abs(differenceY) * ONE_MINUTE_TO_SQUARE;
		final int finalMinutesX = (int) Math.ceil(minutesX)
				% ONE_DEGREE_TO_MINUTE;
		final int finalMinutesY = (int) Math.ceil(minutesY)
				% ONE_DEGREE_TO_MINUTE;
		final int degreeX = (int) (minutesX / ONE_DEGREE_TO_MINUTE);
		final int degreeY = (int) (minutesY / ONE_DEGREE_TO_MINUTE);
		/* setting the first strings */
		final String XAxis = differenceX < 0 ? "west" : "east";
		final String YAxis = differenceY < 0 ? "south" : "north";
	...
	public static Location calculateDiggingPosition(int degree1, int minutes1,
		...
		if (firstHint == "north")
			obsY += (int) Math.ceil((degree1 * ONE_DEGREE_TO_MINUTE + minutes1)
					/ ONE_MINUTE_TO_SQUARE);
		if (firstHint == "south")
			obsY -= (int) Math.ceil((degree1 * ONE_DEGREE_TO_MINUTE + minutes1)
					/ ONE_MINUTE_TO_SQUARE);
		if (firstHint == "east")
			obsX += (int) Math.ceil((degree1 * ONE_DEGREE_TO_MINUTE + minutes1)
					/ ONE_MINUTE_TO_SQUARE);
		if (firstHint == "west")
			obsX -= (int) Math.ceil((degree1 * ONE_DEGREE_TO_MINUTE + minutes1)
					/ ONE_MINUTE_TO_SQUARE);
		...
		if (secondHint == "north")
			obsY += (int) Math.ceil((degree2 * ONE_DEGREE_TO_MINUTE + minutes2)
					/ ONE_MINUTE_TO_SQUARE);
		if (secondHint == "south")
			obsY -= (int) Math.ceil((degree2 * ONE_DEGREE_TO_MINUTE + minutes2)
					/ ONE_MINUTE_TO_SQUARE);
		if (secondHint == "east")
			obsX += (int) Math.ceil((degree2 * ONE_DEGREE_TO_MINUTE + minutes2)
					/ ONE_MINUTE_TO_SQUARE);
		if (secondHint == "west")
			obsX -= (int) ((degree2 * ONE_DEGREE_TO_MINUTE + minutes2) / ONE_MINUTE_TO_SQUARE);

		return new Location(obsX, obsY);
	...
	public static boolean digClue(Player player) {
		final CoordinateData coordinateData = CoordinateData
	...
	public static boolean loadClueInterface(Player player, int itemId) {
		final CoordinateData coordinateData = CoordinateData.forIdClue(itemId);
		if (coordinateData == null)
			return false;
		player.getActionSender()
				.sendInterface(ClueScroll.CLUE_SCROLL_INTERFACE);
		player.getActionSender().sendString(
				putZeroToNumber(coordinateData.getDegree1()) + " degrees "
						+ putZeroToNumber(coordinateData.getMinutes1())
						+ " minutes " + coordinateData.getHint1(), 6971);
		player.getActionSender().sendString(
				putZeroToNumber(coordinateData.getDegree2()) + " degrees "
						+ putZeroToNumber(coordinateData.getMinutes2())
						+ " minutes " + coordinateData.getHint2(), 6972);
		return true;
	}
