...
public class Sextant {
	...
	public static void calculateStartFormula(Player c) {
		...
		final double randomBarDegree = -BAR_LIMIT_RIGHT
				+ c.rotationFactor
				/ DEGREE_FACTOR
				+ Misc.random((int) (BAR_LIMIT_RIGHT + c.rotationFactor
						/ DEGREE_FACTOR + BAR_LIMIT_LEFT + c.rotationFactor
						/ DEGREE_FACTOR));
		// sextant sun randomly placed
		final int limitUpSun = (int) (BAR_LIMIT_LEFT + c.rotationFactor
				/ DEGREE_FACTOR - randomBarDegree);
		final int limitDownSun = Math.abs((int) (randomBarDegree
				+ BAR_LIMIT_RIGHT + c.rotationFactor / DEGREE_FACTOR));
		final int incrementationSun = Misc.random(1) == 0 ? Misc
				.random(limitDownSun) * 2 : -Misc.random(limitUpSun) * 2;
		// sextant landscape randomly placed
		final double randomLandScapeDegree = -(GLOBAL_LIMIT_RIGHT / 5.7)
				+ Misc.random((int) (GLOBAL_LIMIT_RIGHT / 5.7 + GLOBAL_LIMIT_LEFT / 5.7));
		final int limitUpLandScape = (int) (GLOBAL_LIMIT_LEFT / 5.7 - randomLandScapeDegree);
		final int limitDownLandScape = Math
				.abs((int) (randomLandScapeDegree + GLOBAL_LIMIT_RIGHT / 5.7));
		final int incrementationLandScape = Misc.random(1) == 0 ? Misc
				.random(limitDownLandScape) : -Misc.random(limitUpLandScape);
	...
	public static void displayPosition(Player c) {
		final String resultX = CoordinateScrolls.calculateActualPosition(c
				.getPosition().getX(), c.getPosition().getY())[0];
		final String resultY = CoordinateScrolls.calculateActualPosition(c
	...
	public static boolean everythingMatches(Player c) {
		...
		if (!c.getActionAssistant().playerHasItem(CoordinateScrolls.SEXTANT)
				|| !c.getActionAssistant().playerHasItem(
	...
	public static boolean handleSextantButtons(Player c, int buttonId) {
		...
		case 27040:
			...
			return true;
		case 27047: // Sextant give Location
	...
	public static void moveGlobalPiece(Player c, boolean left) {
		...
		final boolean bool1 = c.sextantGlobalPiece > GLOBAL_LIMIT_LEFT;
		final boolean bool2 = c.sextantGlobalPiece < -GLOBAL_LIMIT_RIGHT;
		if (!bool1 && !bool2)
	...
	public static void moveLandScape(Player c, boolean up) {
		...
		final boolean bool1 = c.sextantLandScapeCoords > LANDSCAPE_LIMIT_DOWN;
		final boolean bool2 = c.sextantLandScapeCoords < -LANDSCAPE_LIMIT_UP;
		if (bool1)
	...
	public static void moveLandScapeWithFormula(Player c) {
		c.getActionSender().sendFrame70(0, c.sextantLandScapeCoords,
				SEXTANT_LANDSCAPE);
	}
	...
	public static void moveSextantBar(Player c, boolean left) {
		if (left)
			...
			c.sextantBarDegree -= 1;
		final boolean bool1 = c.sextantBarDegree > BAR_LIMIT_LEFT
				+ c.rotationFactor / DEGREE_FACTOR;
		final boolean bool2 = c.sextantBarDegree < -BAR_LIMIT_RIGHT
				+ c.rotationFactor / DEGREE_FACTOR;
		if (!bool1 && !bool2)
			moveSun(c, left);
		if (bool1)
			c.sextantBarDegree = BAR_LIMIT_LEFT + c.rotationFactor
					/ DEGREE_FACTOR;
		if (bool2)
			c.sextantBarDegree = -BAR_LIMIT_RIGHT + c.rotationFactor
					/ DEGREE_FACTOR;
		updateSextant(c);
	...
	public static void moveSextantBarWithFormula(Player c, double degreeGiven) {
		final double degree = degreeGiven < -BAR_LIMIT_RIGHT + c.rotationFactor
				/ DEGREE_FACTOR ? -BAR_LIMIT_RIGHT + c.rotationFactor
				/ DEGREE_FACTOR : degreeGiven > BAR_LIMIT_LEFT
				+ c.rotationFactor / DEGREE_FACTOR ? BAR_LIMIT_LEFT
				+ c.rotationFactor / DEGREE_FACTOR : degreeGiven;
		final int realDegree = degree > 0 ? (int) (degree * DEGREE_FACTOR)
				: 2047 - (int) (-degree * DEGREE_FACTOR);
		final int x = (int) Math.round(Math.abs(SEXTANT_BAR_LENTH / 2
				* Math.sin(degree * Math.PI / 180)));
		final int y = (int) Math.floor(Math.abs(SEXTANT_BAR_LENTH / 2
				* (1 - Math.cos(degree * Math.PI / 180))));
		c.getActionSender().sendFrame230(SEXTANT_BAR, SEXTANT_ROTATION1,
				realDegree, SEXTANT_BAR_ZOOM);

		c.getActionSender().sendFrame70(
				degree > 0 ? -(x - c.rotationFactor / 2) : x + c.rotationFactor
						/ 2, -y, SEXTANT_BAR);
	...
	private static void moveSextantGlobalPieceWithFormula(Player c) {
		final int degree = c.sextantGlobalPiece < -GLOBAL_LIMIT_RIGHT ? -GLOBAL_LIMIT_RIGHT
				: c.sextantGlobalPiece > GLOBAL_LIMIT_LEFT ? GLOBAL_LIMIT_LEFT
						: c.sextantGlobalPiece;
		final int realDegree = degree > 0 ? degree : 2047 - -degree;
		c.getActionSender().sendFrame230(SEXTANT_TELESCOPE, SEXTANT_ROTATION1,
				realDegree, SEXTANT_GLOBAL_ZOOM);
		c.getActionSender().sendFrame230(SEXTANT_ARC, SEXTANT_ROTATION1,
				realDegree, SEXTANT_GLOBAL_ZOOM);
