...
public class Decanting {
	...
	public enum CombiningDoses {
		...
		public static CombiningDoses getPotionByID(int id) {
			for (final CombiningDoses potion : CombiningDoses.values()) {
		...
		public int getDoseForID(int id) {
			if (id == oneDosePotionID)
				return 1;
			if (id == twoDosePotionID)
				return 2;
			if (id == threeDosePotionID)
				return 3;
			if (id == fourDosePotionID)
				return 4;
			return -1;
		}
	...
	public void potionCombination(Player c, int firstPotID, int secondPotID) {
		final CombiningDoses potion = CombiningDoses.getPotionByID(firstPotID);

		if (potion == null)
			return;

		if (potion.getDoseForID(secondPotID) > 0) {
			final int firstPotAmount = potion.getDoseForID(firstPotID);
			final int secondPotAmount = potion.getDoseForID(secondPotID);

			if (firstPotAmount + secondPotAmount <= 4) {
				c.getActionAssistant().deleteItem(firstPotID,
						c.getActionAssistant().getItemSlot(firstPotID), 1);
				c.getActionAssistant().deleteItem(secondPotID,
						c.getActionAssistant().getItemSlot(secondPotID), 1);
				c.getActionSender().addItem(
						potion.getIDForDose(firstPotAmount + secondPotAmount),
						1);
				c.getActionSender().addItem(EMPTY_VIAL, 1);
			} else {
				final int overflow = firstPotAmount + secondPotAmount - 4;
				c.getActionAssistant().deleteItem(firstPotID,
						c.getActionAssistant().getItemSlot(firstPotID), 1);
				c.getActionAssistant().deleteItem(secondPotID,
						c.getActionAssistant().getItemSlot(secondPotID), 1);
				c.getActionSender().addItem(potion.getIDForDose(4), 1);
				c.getActionSender().addItem(potion.getIDForDose(overflow), 1);
			}
		}
