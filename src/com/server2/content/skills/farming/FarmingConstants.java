...
public class FarmingConstants {// todo farmers watching crops
	...
	public static boolean inRangeArea(Location position1, Location position2,
			Location positionChecked) {
		final int x = position1.getX();
		final int y = position1.getY();
		final int x1 = position2.getX();
		final int y1 = position2.getY();
		return positionChecked.getX() >= x && positionChecked.getY() >= y
				&& positionChecked.getX() <= x1 && positionChecked.getY() <= y1;
	}
}
